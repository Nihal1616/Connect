...
export default function Dashboard() {
  ...
  const handleLikeClick = async (post) => {
    ...
    if (!userId) {
      setLikeMessage("You must be logged in to like a post");
      setShowMessage(true);
    ...
    if (likedBy.includes(userId)) {
      ...
      setLikeMessage("You have already liked this post");
      setShowMessage(true);
      setTimeout(() => setShowMessage(false), 2000);
      return;
    }
    ...
    try {
      ...
      if (resultAction.payload?.message) {
        setLikeMessage(resultAction.payload.message);
        setShowMessage(true);
        setTimeout(() => setShowMessage(false), 2000);
      }
      ...
      setLikeMessage("Something went wrong");
      setShowMessage(true);
      setTimeout(() => setShowMessage(false), 2000);
    }
  };
  ...
  if (authState.user) {
    return (
      <UserLayout>
        <DashboardLayout>
          {showMessage && <div className={styles.likePopup}>{likeMessage}</div>}
                        ...
                        <div>
                          ...
                          <div className={styles.optionsContainer}>
                            <div
                              ...
                              className={styles.singleOption_optionsContianer}
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                fill="none"
